...
def canonicalize_links(word):
    return re.sub("(?P<url>https?://[^\s]+)","LINK", word)

def canonicalize_username(word):
    return re.sub("(@[^\s]+)","ACCOUNT", word)

def canonicalize_hashtag(word):
    return re.sub("(#[^\s]+)","HASHTAG", word)

def canonicalize_word(word, wordset=None, digits=True, links=True, username=True, hashtags=True):
    word = word.lower()
    if digits:
        if (wordset != None) and (word in wordset): return word
        word = canonicalize_digits(word) # try to canonicalize numbers
    if links:
        word = canonicalize_links(word)
    if username:
        word = canonicalize_username(word)
    if hashtags:
